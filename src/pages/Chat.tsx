...
export default function Chat() {
  const { user } = useAuth();
  const { books, messages, loading, sendMessage, deleteMessage, reloadChat } = useApp();
  ...
  return (
    <div className="flex h-[calc(100vh-7rem)] flex-col overflow-hidden">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between animate-fade-in mb-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Chat</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            O'quvchilar va kutubxona xodimlari uchun umumiy suhbat. Qo'pol so'z yuborish taqiqlanadi.
          </p>
        </div>
      </div>

      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900 animate-fade-in">
        <div
          ref={scrollRef}
          className="flex-1 space-y-4 overflow-y-auto bg-slate-50 p-4 dark:bg-slate-950/40"
        >
            ...
            sorted.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`group max-w-[75%] rounded-2xl px-3.5 py-2.5 shadow-sm ${
                      own
                  ...
                  >
                    {!own && (
                      <p
                        className={`mb-0.5 text-xs font-semibold ${
                          own ? '' : 'text-primary-600 dark:text-primary-400'
                        }`}
                      >
                        {m.userName}
                        <span className="ml-1.5 font-normal text-slate-400 dark:text-slate-500">
                          {roleLabel[m.userRole] ?? m.userRole}
                          {m.userRole === 'student' && m.grade ? ` · ${m.grade}-sinf` : ''}
                        </span>
                      </p>
                    )}
                      ...
                      >
                        <span
                          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md ${
                            own ? 'bg-primary-100/20' : 'bg-primary-100 dark:bg-primary-900/40'
                          }`}
                        >
                          {m.bookCover ? (
                            <img
                            ...
                            <BookMarked
                              className={`h-4 w-4 ${
                                own ? 'text-white' : 'text-primary-600 dark:text-primary-400'
                              }`}
                            />
                          )}
                        </span>
                        <span className="min-w-0">
                          ...
                          {m.bookAuthor && (
                            <span className="block truncate opacity-80">{m.bookAuthor}</span>
                          )}
                        </span>
                      </div>
                    )}
                    ...
                    <p
                      className={`mt-1 flex items-center gap-1.5 text-[11px] ${
                        own ? 'text-primary-100' : 'text-slate-400 dark:text-slate-500'
                      }`}
                    >
        ...
        <div className="border-t border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900">
                ...
                {booksFiltered.length === 0 ? (
                  ...
                  booksFiltered.map((b) => (
                    ...
                    >
                      ...
                      <span className="shrink-0 text-xs text-slate-400">{b.author}</span>
                    </button>
                  ))
                )}
              </div>
          ...
          <div className="flex items-end gap-2">
            ...
            />
            <Button onClick={() => void handleSend()} loading={sending} className="shrink-0">
