...
interface ThemeToggleProps extends Omit<ButtonHTMLAttributes<HTMLButtonElement>, 'onChange'> {
  checked: boolean;
  onChange: (checked: boolean) => void;
}

const EASE = { transition: 'all 0.25s cubic-bezier(0.4, 0, 0.2, 1)' };

export default function ThemeToggle({ checked, onChange, ...props }: ThemeToggleProps) {
  return (
    <button
      type="button"
      role="switch"
      ...
      className="shrink-0 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 transition-transform duration-200 hover:scale-105 active:scale-95 dark:focus-visible:ring-primary-400"
      {...props}
    >
      <svg
        ...
        className="h-8 w-[3.2rem] overflow-visible"
      >
        <defs>
          <filter id="container" x="-25%" y="-25%" width="150%" height="150%">
            <feDropShadow dx="1.5" dy="2.5" stdDeviation="2.5" floodColor="#2b4360" floodOpacity="0.35" />
          </filter>
          <filter id="sun-outer" x="-40%" y="-40%" width="180%" height="180%">
            <feDropShadow dx="0" dy="0" stdDeviation="4" floodColor="#f8e664" floodOpacity="0.55" />
          </filter>
          <filter id="sun" x="-40%" y="-40%" width="180%" height="180%">
            <feDropShadow dx="-1" dy="2" stdDeviation="2" floodColor="#c9a227" floodOpacity="0.35" />
          </filter>
          <filter id="moon" x="-40%" y="-40%" width="180%" height="180%">
            <feDropShadow dx="0" dy="0" stdDeviation="3.5" floodColor="#a6cad0" floodOpacity="0.5" />
          </filter>
          <filter id="cloud" x="-25%" y="-25%" width="150%" height="150%">
            <feDropShadow dx="0" dy="2" stdDeviation="2.5" floodColor="#83cbd8" floodOpacity="0.4" />
          </filter>
        </defs>
        ...
        <g transform="translate(3.5 3.5)">
          ...
          <g
            style={{
              transform: checked ? 'translate(28px, 2.333px)' : 'translate(2.333px, 2.333px)',
              ...EASE,
            }}
          >
            {/* Sun */}
            <g style={{ opacity: checked ? 0 : 1, ...EASE }}>
            ...
            {/* Moon */}
            <g style={{ opacity: checked ? 1 : 0, ...EASE }}>
          ...
          <g filter="url(#cloud)" transform="matrix(1, 0, 0, 1, -3.5, -3.5)">
            <path
              ...
              style={{ opacity: checked ? 0 : 1, ...EASE }}
            />
          </g>

          {/* Stars (night only) */}
          <g fill="#def8ff" transform="translate(3.585 1.325)" style={{ opacity: checked ? 1 : 0, ...EASE }}>
