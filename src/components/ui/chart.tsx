...
function resolveLegendColor(color: string | undefined): string | undefined {
  if (!color) return undefined;
  if (!color.startsWith("url(#")) return color;
  const id = color.slice(5, -1);
  const el = typeof document !== "undefined" ? document.getElementById(id) : null;
  const stop = el?.querySelector("stop");
  const resolved = stop?.getAttribute("stop-color");
            ...
            >
              {itemConfig?.icon && !hideIcon ? (
                ...
                <div
                  ...
                  style={{
                    backgroundColor: resolveLegendColor(item.color),
                  }}
                />
              )}
              {itemConfig?.label}
